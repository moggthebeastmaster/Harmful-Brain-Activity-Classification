...
class EfficientNet(nn.Module):
    def __init__(self, config:EfficientNetConfig, pretrain:bool):
        super().__init__()
        self.config = config
        if self.config.model_framework in ["efficientnet_b0", "eeg_efficientnet_b0", "eeg_efficientnet_b0_v2"]:
        ...
        if self.config.model_framework in ["efficientnet_b7", "eeg_efficientnet_b7", "eeg_efficientnet_b7_v2"]:
